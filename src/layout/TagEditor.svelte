...
<InputMultiBlock>
  {#each dataset.tagGroups as group, i}
   <InputMultiBlock dense={true}>
      ...
      {:else}
        <span class="text-xs font-bold text-zinc-300 block mb-1">{group.name}
        ...
        {#each parseTags(group.tags) as tag}
          <button class="text-xs font-bold text-zinc-900 bg-zinc-600 rounded-md px-2 mr-1 mb-0.5">{tag}</button>
        {/each}
      {/if}
    </InputMultiBlock>
